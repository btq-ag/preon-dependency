...
void fri_ldt(sha3_context *hashchain_ctx, MerkleTree **merkle_trees, uint8_t **mt_roots,
             ...
             Oracle **f_i_oracles, uint64_t *const multi_f_i_evaluations_by_interaction, const Parameters *params)
{
    ...
    for (size_t i = 0; i < params->fri_num_reductions; ++i)
    {
        ...
        if (i > 0)
        {
            ...
            signal_prover_round_done(hashchain_ctx, merkle_trees[i + 1], mt_roots[i + 1],
                                     merkle_tree_contents, 1,
                                     NULL, 0,
                                     params);
        }

        /* For each interaction, receive the verifier challenge and create f_{i + 1} */
        uint64_t *x_i = (uint64_t *)malloc(params->field_bytesize);
        squeeze_field_elements(hashchain_ctx, x_i, 1, params->field_words, params->field_bytesize);
        ...
        poly_eval(unshifted_vp_x, unshifted_vp, unshifted_vp_len, x_i, params->field_words);
        uint64_t *inv_vp_linear_term = (uint64_t *)malloc(params->field_bytesize);
        field_inv(inv_vp_linear_term, &unshifted_vp[1 * params->field_words], params->field_words);
        ...
        for (size_t j = 0; j < num_cosets; j++)
        {
            ...
            for (size_t k = 0; k < coset_size; k++)
            {
                ...
                if ((x_in_domain) && field_equal(x_i, tmp, params->field_words))
                {
                ...
                memcpy(&shifted_coset_elements[k * params->field_words], x_i, params->field_bytesize);
                field_subEqual(&shifted_coset_elements[k * params->field_words], tmp, params->field_words);
            }
            if (!x_in_domain)
            {
                uint64_t k[params->field_words];
                field_mul(k, inv_vp_linear_term, shifted_vp_x, params->field_words);
                ...
                for (size_t k = 0; k < coset_size; k++)
                {
                    ...
                    field_mul(tmp, &cur_f_i[(j * coset_size + k) * params->field_words], &lagrange_coefficients[k * params->field_words], params->field_words);
    ...
    signal_prover_round_done(hashchain_ctx, NULL, NULL,
                             NULL, 0,
                         ...
                         const Parameters *params)
{
    ...
    for (size_t q = 0; q < query_sets_len; q++)
    {
        ...
        for (size_t i = 0; i < params->fri_num_reductions; ++i)
        {
            ...
            memcpy(x_i, &verifier_messages[(24 + i) * params->field_words], params->field_bytesize);
            ...
            field_inv(c, &localizer_poly[1 * params->field_words], params->field_words);
            ...
            if (!interpolation_done)
            {
                ...
                field_mul(lagrange_coefficients_mul, vp_x, c, params->field_words);
                ...
                {
                    field_mul(tmp, &lagrange_coefficients[k * params->field_words], &fi_on_si_coset[k * params->field_words], params->field_words);
                    field_addEqual(interpolation, tmp, params->field_words);
                }
            }
